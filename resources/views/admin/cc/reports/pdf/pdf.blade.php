...
<div class="el-no3">
    <span>{{trans('admin.disclosure_of_balances_of_accounts_of_cost_centers')}}</span>
</div>
        ...
        @foreach ($glcc as $merged){
            <tr>
                ...
                <td>
                    <div style="display:none">
                        @if($merged->type == '0')
                        {{$creditor = sumallcc2($merged->id,$from,$to,'creditor','<')}}
                        {{$debtor = sumallcc2($merged->id,$from,$to,'debtor','<')}}
                        @else
                            {{$creditor = allcc2($merged->id,$from,$to,'creditor','<')}}
                            {{$debtor = allcc2($merged->id,$from,$to,'debtor','<')}}
                        @endif
                ...
                <td>
                    <div style="display:none">
                        @if($merged->type == '0')
                        {{$creditor1 = sumallcc2($merged->id,$from,$to,'creditor','<')}}
                        {{$debtor1 = sumallcc2($merged->id,$from,$to,'debtor','<')}}
                        @else
                            {{$creditor1 = allcc2($merged->id,$from,$to,'creditor','<')}}
                            {{$debtor1 = allcc2($merged->id,$from,$to,'debtor','<')}}
                        @endif
                ...
                <td>
                    @if($merged->type == '0')
                    {{$debtor2 = sumallcc($merged->id,$from,$to,'debtor','>=')}}
                    <div style="display:none">
                        {{$dataDebtor1 += sumallcc($merged->id,$from,$to,'debtor','>=')}}
                    </div>
                        @else
                        {{$debtor2 = allcc($merged->id,$from,$to,'debtor','>=')}}
                        <div style="display:none">
                            {{$dataDebtor1 += allcc($merged->id,$from,$to,'debtor','>=')}}
                        </div>
                    @endif
                </td>
                <td>
                    @if($merged->type == '0')
                    {{$creditor2 = sumallcc($merged->id,$from,$to,'creditor','>=')}}
                    <div style="display:none">
                        {{$dataCredit1 += sumallcc($merged->id,$from,$to,'creditor','>=')}}
                    </div>
                        @else
                        {{$creditor2 = allcc($merged->id,$from,$to,'creditor','>=')}}
                        <div style="display:none">
                            {{$dataCredit1 += allcc($merged->id,$from,$to,'creditor','>=')}}
                        </div>
                    @endif
                </td>
                <td>
