@extends('admin.index')
@section('title', 'الانشطه التجاريه للعملاء')
@section('content')


    @include('admin.layouts.message')

    <form action="{{route('AstNutrbusn.store')}}" method="POST">
        {{ csrf_field() }}
        <div class="panel panel-primary" style="width:50%; margin:auto auto;    ">
            <div class="panel-heading">
                <div class="panel-title">
                    الانشطه التجاريه للعملاء
                </div>
            </div>
            <div class="panel-body">
                ...
                <div class="row">
                    <br><br>
                    <label for="Nutr_No" class="col-md-2">{{trans('admin.Nutr_No')}}</label>
                    <input type="text" name="Nutr_No" id="Nutr_No" class="form-control col-md-2" readonly value="{{$last}}">
                </div>
                {{-- نهاية رقم النشاط --}}
                {{-- اسم الحساب عربى --}}
                <div class="row">
